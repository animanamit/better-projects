    ...
    <div className="p-2 sm:p-4">
      ...
      {isMobile && (
        <div className="flex mb-4 border-b border-black/10">
          ...
          >
            Recent Files{" "}
            {recentUploads.length > 0 && `(${recentUploads.length})`}
          </button>
        </div>
      )}
      ...
      {/* Upload section */}
      <div
        className={`bg-[#f8f8f8] p-3 sm:p-4 mb-6 ${
          isMobile && activeTab !== "upload" ? "hidden" : ""
        }`}
      >
      ...
      {recentUploads.length > 0 && (
        <div
          className={`bg-[#f8f8f8] p-3 sm:p-4 ${
            isMobile && activeTab !== "recent" ? "hidden" : ""
          }`}
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {recentUploads.map((file) => (
              <div
                key={file.id}
                className="bg-white p-3 hover:bg-[#f0f0f0] rounded-sm"
              >
